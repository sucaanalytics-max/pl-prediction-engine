...
6. **Verify.** Run `PYTHONPATH=. .venv/bin/python -m unittest discover -s pipeline/tests -v`. Use the repo venv: CLAUDE.md records that bare `python3` is a Homebrew 3.14 without scipy, which yields 56 import errors and a short test count that reads as a code regression rather than a missing interpreter. Do not pipe to `tail`, which reports tail's exit code so a failing suite looks like a pass — `pipeline/tests/test_contracts.py` includes `FeatureContractTests`, `SimulationContractTests`, and `ForwardValidationTests` that guard your area. A full pipeline run needs PyMC sampling and takes a long time; prefer targeted scripts over full runs while iterating.
