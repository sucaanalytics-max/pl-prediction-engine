...
export function StatsTable({
  ...
  const visible = useMemo(() => {
    ...
    const column = columns.find((c) => c.key === (sortKey ?? question.sortKey)) ?? columns[0];
  ...
  }, [rows, deferredQuery, show, sortKey, columns, question.sortKey]);
              ...
              <div style={{
                ...
                {columns.map((column) => (
                  <button
                    ...
                    aria-pressed={(sortKey ?? question.sortKey) === column.key}
                    style={{
                      height: 30, display: "flex", alignItems: "center",
                      justifyContent: "flex-end", paddingRight: 10,
                      background: (sortKey ?? question.sortKey) === column.key
