...
//
// Four narrowers and their types — `narrowTable`/`Standing`, `narrowLatest` with
// the whole value-bet layer (`Bet`, `Prediction`, `halfKellyOf`, `devigStatusOf`,
// `narrowBets`), `narrowH2H`/`H2HEntry`/`H2HGame`, and
// `narrowBlendWeight`/`BlendWeight` — were left behind when the dashboard narrowed
// to one team. Nothing imported them and no REGISTRY entry named them, so they
// were unreachable: a narrower with no descriptor cannot be reached by
// `useArtifact`, which is the only door into this layer.
//
// They were not harmless. A narrowing envelope earns its keep by being the single
// answer to "what shape is this file", and four narrowers for files the app never
// opens made that claim untrue — a reader adding a fifth had to work out which of
// the nine were live. Deleted rather than commented out, because git remembers and
// a comment does not typecheck.
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// matches.json
// ─────────────────────────────────────────────────────────────────────────────
